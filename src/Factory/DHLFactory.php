...
class DHLFactory implements DHLConnectionFactoryInterface, DHLSenderFactoryInterface {
  ...
  public function createParcel(IsotopePackagingSlipModel $packagingSlip): void {
    ...
    $recipient = [
      'first_name' => html_entity_decode(Format::dcaValue(IsotopePackagingSlipModel::getTable(), 'firstname', $packagingSlip->firstname)),
      'last_name' => html_entity_decode(Format::dcaValue(IsotopePackagingSlipModel::getTable(), 'lastname', $packagingSlip->lastname)),
      'street' => html_entity_decode(Format::dcaValue(IsotopePackagingSlipModel::getTable(), 'street_1', $packagingSlip->street_1)),
      'number' => html_entity_decode(Format::dcaValue(IsotopePackagingSlipModel::getTable(), 'housenumber', $packagingSlip->housenumber)),
      'postal_code' => html_entity_decode(Format::dcaValue(IsotopePackagingSlipModel::getTable(), 'postal', $packagingSlip->postal)),
      'city' => html_entity_decode(Format::dcaValue(IsotopePackagingSlipModel::getTable(), 'city', $packagingSlip->city)),
      'cc' => html_entity_decode(Format::dcaValue(IsotopePackagingSlipModel::getTable(), 'country', strtoupper($packagingSlip->country))),
    ];
    if ($packagingSlip->street_2 || $packagingSlip->street_3) {
      $recipient['additional_address_line'] = trim(implode(" ", [html_entity_decode(Format::dcaValue(IsotopePackagingSlipModel::getTable(), 'street_2', $packagingSlip->street_2)), html_entity_decode(Format::dcaValue(IsotopePackagingSlipModel::getTable(), 'street_3', $packagingSlip->street_3))]));
    }
    ...
    if ($packagingSlip->company && empty($packagingSlip->dhl_servicepoint_id)) {
      $recipient['company_name'] = html_entity_decode(Format::dcaValue(IsotopePackagingSlipModel::getTable(), 'company', $packagingSlip->company));
    }
    $parcel = new Parcel([
